...
public class QuadTree<Terminal> {
    ...
    public void setColor(int size, PApplet c){
        switch(size){
            case 1:
                c.stroke(0, 255, 0);
                break;
    ...
    private interface QuadTreeNode<Terminal> {
        ...
        void draw(PApplet c, int x, int y, int size);
    ...
    private class QuadTreeContainer implements QuadTreeNode<Terminal> {
        ...
        public void draw(PApplet c, int x, int y, int size){
            int subSize = size / 2;
            if(contents[0] != null) contents[0].draw(c, x, y, subSize);
            if(contents[1] != null) contents[1].draw(c, x, y + subSize, subSize);
            if(contents[3] != null) contents[3].draw(c, x + subSize, y, subSize);
            if(contents[2] != null) contents[2].draw(c, x + subSize, y + subSize, subSize);
            setColor(size, c);
            c.rect(x * 30, y * 30, 30 * size, 30 * size);
        }
    ...
    private class TerminalNode implements QuadTreeNode<Terminal> {
        ...
        public void draw(PApplet c, int x, int y, int size) {
            setColor(size, c);
            c.rect(x * 30, y * 30, 30, 30);
            c.text(item.toString(), x * 30 + 15, y * 30 + 15);
        }
